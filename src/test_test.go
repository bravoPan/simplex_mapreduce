...
func port(suffix string) string {
  s := "/var/tmp/824-"
  s += strconv.Itoa(os.Getuid()) + "/" //return the current process id
  os.Mkdir(s, 0777)
  s += "mr"
  s += strconv.Itoa(os.Getpid()) + "-"
  s += suffix
  // the final master address is: /var/tmp/824-<pid>/mr-master
  fmt.Println(s)
  return s
}

func setup() *MapReduce {
  file := makeInput() // make files with 1-100000 in every single line...
  master := port("master")
...
func ReduceFunc(key string, values *list.List) string {
  ...
  for e := values.Front(); e != nil; e = e.Next() {
    ...
    if err != nil{
        log.Fatal("something wrong with ReduceFunc")
    }
    vCnt += v
    // DPrintf("Reduce %s %v\n", key, e.Value)
  }
  return strconv.Itoa(vCnt)
}
...
// func TestDoMapFunc(t *testing.T){
//     DoMap(0, "824-mrinput.txt", 50, MapFunc)
// }
//
// func TestDoRedFunc(t *testing.T){
//     DoReduce(0, "824-mrinput.txt", 1, ReduceFunc)
// }


// func TestMapRedFunc(t *testing.T){
//     res := MapFunc("I am a very stink pig")
//     ReduceFunc("1", res)
// }
// func TestSplit(t *testing.T) {
//     file := makeInput()
//     master := port("master")
//     mr := InitMapReduce(nMap, 1, file, master)
//     // mr.Split(mr.file)
//     mr.Merge()
// }
